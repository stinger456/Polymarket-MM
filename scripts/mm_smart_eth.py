#!/usr/bin/env python3
"""
HYBRID Market Maker - ETH Hourly (Adaptive)
...
class HybridMM_ETH:
    def __init__(self):
        ...
        # Volatility tracking (ETH prices over last 5 mins)
        self.eth_prices = deque(maxlen=60)  # Store last 60 price checks
        self.last_eth_price = None
        self.volatility = "LOW"  # LOW, MEDIUM, HIGH

        print(f"✓ Connected (ETH MM): {safe_address[:20]}...")

    def get_markets(self) -> List[Dict]:
        """Get active ETH hourly markets."""
        markets = []
        ...
        for offset in range(4):
            ...
            # ETH hourly market slug format
            slug = f"ethereum-up-or-down-{month}-{day}-{h}-et"
    ...
    def get_eth_price(self) -> Optional[float]:
        ...
        try:
            with httpx.Client(timeout=5) as http:
                resp = http.get("https://api.binance.com/api/v3/ticker/price", params={"symbol": "ETHUSDT"})
                return float(resp.json()["price"])
        except:
            return None

    def update_volatility(self):
        """Calculate volatility based on recent ETH price movement."""
        ...
        prices = list(self.eth_prices)
        min_p, max_p = min(prices), max(prices)
        range_pct = (max_p - min_p) / min_p * 100

        # Classify volatility (ETH can be slightly more volatile than BTC)
        if range_pct > 0.4:  # >0.4% move in recent period
            self.volatility = "HIGH"
        elif range_pct > 0.15:  # 0.15-0.4% move
    ...
    def execute_taker_taker(self, market: Dict, yes_ask: float, no_ask: float) -> bool:
        ...
        print(f"\n{'='*60}")
        print(f"⚡ ETH TAKER-TAKER TRADE - {market['hour']} ET (INSTANT FILL)")
        print(f"{'='*60}")
    ...
    def execute_hybrid(self, opp: Dict) -> bool:
        ...
        print(f"\n{'='*60}")
        print(f"🚀 ETH HYBRID TRADE - {m['hour']} ET")
        print(f"{'='*60}")
    ...
    def run(self):
        """Main loop."""
        print(f"\n{'='*60}")
        print("ETH HYBRID MARKET MAKER (PROFITABLE)")
        ...
        while True:
            try:
                ...
                if not markets:
                    print(f"\r[{ts}] No ETH markets", end="", flush=True)
                    time.sleep(2)
                    continue
                ...
                print(f"\r[{ts}] ETH #{scan} | {vol_indicator}>{min_profit_now:.1f}¢ | {status} | P&L: ${self.total_pnl:+.2f}   ", end="", flush=True)
